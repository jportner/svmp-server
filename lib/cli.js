...
program
    ...
    .description('Create a volume for the user based on the gold snapshot id in config-local')
    .action(function(un) {
        console.log('');
...
program
    ...
    .description('Associate an existing user data volume with the specified user.')
    .action(function(un, volid) {
        ...
        users.getUser(un, function(err, user) {
            if(user) {
                ...
                user.save(function (err1, updateduser) {
                    if(updateduser) {
                        console.log("        Data volume associated with user! ID: ", updateduser.volume_id );
                        mongoose.connection.close();
                    } else {
                        console.log('        ERROR: ', err1);
                        mongoose.connection.close();
